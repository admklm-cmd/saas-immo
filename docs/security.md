...
- **Soumission native avant l'hydratation, corrigée (26/09/2026)** : un formulaire client sans
  attribut `method` était envoyé en GET par le navigateur si l'utilisateur validait avant le chargement
  de JavaScript (email et mot de passe de connexion vus dans l'URL lors d'un test local). Tous les
  `<form>` déclarent désormais `method="post"` (ou `method="get"` explicite pour les filtres sans
  donnée personnelle). Garde automatique : `components/form-method.guard.test.ts` ; parcours sans
  JavaScript : `e2e/soumission-avant-hydratation.spec.ts`.
...
| 2026-09-23 | **Branche `feat/particle-animations`** (`git diff feat/complete-demo...HEAD`, 83 fichiers, purement frontend) : moteur de particules Canvas 2D (`components/motion/**`), fond `RouteParticles` dans `app/(app)/layout.tsx`, galerie `/dev/particles` (`?fond=`, `?only=`), micro-interactions (`Button`, `Alert`, `AnimatedErrorState`, `retryable.ts`, `use-single-flight.ts`, `SimulationBadge`), cartes d'agents, formulaires estimation/connexion | Aucun problème critique, élevé ou moyen. `package.json`/`package-lock.json` inchangés, aucun fichier `actions`/`queries`/`data`/migration/`lib/` modifié, en-têtes et CSP (`lib/security/headers.ts`, `next.config.ts`) inchangés et compatibles (canvas 2D : ni `blob:`, ni `eval`, ni ressource externe). Moteur : aucun `fetch`, URL externe, stockage navigateur, `innerHTML` ni lecture de données ; canvas `aria-hidden`, `pointer-events: none`, sans libellé, forme choisie par la seule route (jamais par l'état d'un agent). Galerie : garde `isAnimationGalleryEnabled` avant lecture des paramètres ; `npm run build` la prérend en **404 statique** (`.meta` : `status 404`) quel que soit `?fond=` ; `fond`/`only` validés contre la liste fermée `PARTICLE_PRESETS`. Doubles soumissions bloquées par `useSingleFlight` (verrou ref) ; « Réessayer » limité aux codes techniques (liste fermée) ; un réessai d'envoi reste protégé par la base (`outbound_message_already_sent`) et un réessai vers « Mandat signé » rouvre la confirmation humaine. Badges Simulation toujours écrits (seul le point pulse). **1 test ajouté** : `app/dev/particles/page.test.tsx` (404 en production pour toute requête, rejet des valeurs hors liste). Reste faible : le code client de la galerie est présent dans les fichiers statiques du build (inaccessible, sans donnée ni secret). `tsc`, lint propres, Vitest vert, `npm audit --omit=dev` : 0. Playwright non lancé (orchestrateur). Livraison autorisée. |
| 2026-09-26 | **Branche `feat/typography-particles`** (lot UI : polices Geist/Inter/Geist Mono auto-hébergées, fond de particules plein écran, voiles de lisibilité) | Aucun problème critique ou élevé ouvert. Polices servies depuis l'origine (0 requête externe, CSP inchangée). Mentions légales et garde-fous toujours visibles. Contraste mesuré au pixel : minimum 5,41:1 après ajout de `.particle-veil-tight`. **Corrigé** : soumission native en GET avant hydratation (connexion — élevé ; estimation et compte-rendu — faibles), garde automatique + E2E sans JavaScript. `npm audit` : 0. Livraison autorisée. |
